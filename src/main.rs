...
use aes_ctr::stream_cipher::{
    NewFixStreamCipher, StreamCipherCore
};
use ring::aead;
...
fn decrypt_pp_payload<'a>(key: &[u8], iv: &[u8], pn: &[u8], ad: &[u8], payload: &'a mut [u8]) -> &'a mut [u8] {
...
fn decrypt_hs_payload<'a>(key: &[u8], iv: &[u8], pn: &[u8], ad: &[u8], payload: &'a mut [u8]) -> &'a mut [u8] {
    let nonce = xor(iv, &left_pad(pn, iv.len())[..]);
...
fn decode_variable_length_integer(v: &[u8]) -> (u64, usize) {
    ...
    if mlb == 0 {
        ...
        ((((v[0] & 0b0011_1111) as u64) << 24) + ((v[1] as u64) << 16) + ((v[2] as u64) << 8) + (v[3] as u64), 4)
    } else {
        ((((v[0] & 0b0011_1111) as u64) << 56) + ((v[1] as u64) << 48) + ((v[2] as u64) << 40) + ((v[3] as u64) << 32) + ((v[4] as u64) << 24) + ((v[5] as u64) << 16) + ((v[6] as u64) << 8) + (v[7] as u64), 8)
    }
}

fn decode_print(sender: Side, packet: &[u8]) {
    ...
    if packet[0] >> 7 == 1 {
        ...
        match sender {
            Side::Client => {
                ...
                offset += 18;
                //
                let client_ap_key = hex!("7dae294fd8bb4264b985082afc8f19571810e72ff7f61427ab239017b06d4542");
                let client_ap_iv = hex!("0c5b8b38e76512e6a20857a4");
                let client_ap_pn = hex!("2a50a23b75bd70afa9566830b56113422b83bc2475b8696ad8d8938adffdc6a1");
                // Packet Number
                let mut pn = [0; 4];
                pn.copy_from_slice(&packet[offset..(offset + 4)]);
                let sample = &packet[(offset + 4)..(offset + 4 + 16)];
                ...
                let mut payload = packet[offset + amt..].to_vec();
                let mut payload = payload.as_mut_slice();
                let payload = decrypt_hs_payload(&client_ap_key, &client_ap_iv, &pn, &ad, &mut payload);
                print_hex("Payload", &payload);
            }
            Side::Server => {
                // Dst Conn ID (17)
                ...
                offset += 17;
                //
                let server_ap_key = hex!("13cfbc7d41fd53f2228f529cbadc2e8bb3bb70d5c2dbac03fae02c63248d77c8");
                let server_ap_iv = hex!("3316b6c7d4b85cf5bcb3fbf5");
                let server_ap_pn = hex!("337940cda868c035431596f366ca4fc00a1008f92013c4364c0f5d2d97b1ca8d");
                // Packet Number
                let mut pn = [0; 4];
                pn.copy_from_slice(&packet[offset..(offset + 4)]);
                let sample = &packet[(offset + 4)..(offset + 4 + 16)];
                ...
                let mut payload = packet[offset + amt..].to_vec();
                let mut payload = payload.as_mut_slice();
                let payload = decrypt_hs_payload(&server_ap_key, &server_ap_iv, &pn, &ad, &mut payload);
                print_hex("Payload", &payload);
            }
        }
